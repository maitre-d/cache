...
class Cache {
    constructor(options, drivers = []) {
        ...
        this.options = Object.assign(Object.assign({ omit_replication: [] }, options), { driver_methods: Object.assign({ default: 'omit', fetch: [], set: [], delete: [], flush: [], cleanup: [] }, (options.driver_methods || {})) });
    }
    ...
    fetch(key, cb, options) {
        return __awaiter(this, void 0, void 0, function* () {
            options = Object.assign({ segments: [], force_new: false }, options);
            let value;
            const misses = [];
            const drivers = this.get_active('fetch');
            if (!options.force_new) {
                for (const driver of drivers) {
                    [value] = yield Promise.all([driver.get(key)]);
    ...
    cleanup() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return Promise.all(drivers.map((driver) => driver.cleanup())).then((results) => Math.max(...results));
        });
    }
    flush() {
        return __awaiter(this, void 0, void 0, function* () {
